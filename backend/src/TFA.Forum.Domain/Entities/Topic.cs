...
public class Topic : Entity<TopicId>, IAuditable
{
    ...
    private Topic(TopicId id, ForumId forumId, UserId userId, Title title, Content content, DateTimeOffset createdAt) : base(id)
    {
        ForumId = forumId;
        UserId = userId;
        Title = title;
    ...
    public static Topic Create(TopicId id, ForumId forumId, UserId userId, Title title, Content content, DateTimeOffset createdAt )
    {
        return new Topic(id, forumId, userId, title, content, createdAt);
    }
}
